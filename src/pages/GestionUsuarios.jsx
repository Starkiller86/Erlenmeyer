...
const emptyForm = { username: '', email: '', nombre_completo: '', password: '', rol: 'usuario', activo: 1 };
...
export default function GestionUsuarios() {
    ...
    const openEdit = (u) => {
        setEditTarget(u);
        setForm({ username: u.username, email: u.email, nombre_completo: u.nombre_completo, password: '', rol: u.rol, activo: u.activo });
        setShowModal(true);
    };

    const handleSave = async () => {
        if (!form.username || !form.email) return showMsg('warning', 'Username y email son requeridos.');
        if (!editTarget && !form.password) return showMsg('warning', 'La contraseña es requerida al crear un usuario.');
        setSaving(true);
        try {
            if (editTarget) {
                await authFetch(`/usuarios/${editTarget.id}`, { method: 'PUT', body: JSON.stringify(form) });
                showMsg('success', 'Usuario actualizado.');
            } else {
                await authFetch('/usuarios', { method: 'POST', body: JSON.stringify(form) });
    ...
    const handleDelete = async (u) => {
        ...
        try {
            await authFetch(`/usuarios/${u.id}`, {
                method: 'PUT',
                body: JSON.stringify({ ...u, activo: 0, password: '' })
            });
                        ...
                        <tbody>
                            {usuarios.map((u, i) => (
                                <tr key={u.id} style={{ borderBottom: '1px solid #f3f0ff', background: i % 2 === 0 ? '#fff' : '#faf8ff' }}>
                                    ...
                                    <td style={{ padding: '0.7rem 1rem', fontWeight: 600, color: '#1a0533' }}>{u.username}</td>
                ...
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(26,5,51,0.6)', backdropFilter: 'blur(3px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1050, padding: '1rem' }}
                    ...
                    <div style={{ background: '#fff', borderRadius: 14, width: '100%', maxWidth: 480, boxShadow: '0 24px 64px rgba(0,0,0,0.3)' }}
                        ...
                        <div style={{ background: 'linear-gradient(135deg,#5e35b1,#7b1fa2)', color: '#fff', padding: '1rem 1.25rem', borderRadius: '14px 14px 0 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontWeight: 600 }}>
                            <span>{editTarget ? `Editar: ${editTarget.username}` : 'Nuevo Usuario'}</span>
                        ...
                        <div style={{ padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '0.875rem' }}>
                            {[
                                ...
                                { label: editTarget ? 'Nueva contraseña (dejar vacío para no cambiar)' : 'Contraseña *', key: 'password', type: 'password' },
                            ].map(({ label, key, type }) => (
                                <div key={key}>
                            ...
                            <div style={{ display: 'flex', gap: '1rem' }}>
                                ...
                                <div style={{ flex: 1 }}>
                                    ...
                                    <select value={form.activo} onChange={e => f('activo', parseInt(e.target.value))}
                                        style={{ width: '100%', padding: '0.45rem 0.65rem', border: '1px solid #d1c4e9', borderRadius: 6, fontSize: '0.875rem' }}>
