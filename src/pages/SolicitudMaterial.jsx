...
export default function SolicitudMaterial() {
  ...
  const [tab, setTab] = useState('nueva'); // 'nueva' | 'mis' | 'admin'
  const [form, setForm] = useState({ practice_name: '', subject: '', group_name: '', schedule: '', practice_date: '' });
  ...
  const loadRequests = async (admin = false) => {
    ...
    try {
      const data = admin
        ? await authFetch('/loan-requests')
        : await authFetch(`/loan-requests?user_id=${user.id}`);
      setRequests(data);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await authFetch('/loan-requests', {
        method: 'POST',
        body: JSON.stringify({ user_id: user.id, user_nombre_completo: user.nombre_completo, ...form, materials }),
      });
  ...
  const handleStatus = async (id, status) => {
    try {
      await authFetch(`/loan-requests/${id}/status`, { method: 'PATCH', body: JSON.stringify({ status }) });
          ...
          {loadingList ? (
            ...
            <div key={req.id} className="sol-item">
              ...
              <div className="sol-item-actions">
                <button className="sol-btn-eye" onClick={() => setSelected(req)}>
                  Ver
                </button>
